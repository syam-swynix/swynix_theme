frappe.ready(function() {
    // Customize page title
    $('title').text('Swynix - Login');
    
    // Add Swynix branding text
    if ($('.login-content .page-card-head').length) {
        $('.page-card-head-title').text('Login');
    }
    
    // Add password toggle functionality
    addPasswordToggle();
});

function addPasswordToggle() {
    // Wait for password field to be rendered
    setTimeout(function() {
        const passwordField = $('#login_password');
        
        if (passwordField.length && !passwordField.next('.password-toggle').length) {
            // Create toggle button
            const toggleButton = $('<span class="password-toggle">' +
                '<svg class="icon-eye" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">' +
                    '<path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path>' +
                    '<circle cx="12" cy="12" r="3"></circle>' +
                '</svg>' +
                '<svg class="icon-eye-off" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="display:none;">' +
                    '<path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path>' +
                    '<line x1="1" y1="1" x2="23" y2="23"></line>' +
                '</svg>' +
            '</span>');
            
            // Add toggle button after password field
            passwordField.parent().css('position', 'relative');
            passwordField.after(toggleButton);
            
            // Toggle password visibility
            toggleButton.on('click', function() {
                const type = passwordField.attr('type') === 'password' ? 'text' : 'password';
                passwordField.attr('type', type);
                
                // Toggle icons
                $(this).find('.icon-eye').toggle();
                $(this).find('.icon-eye-off').toggle();
            });
        }
    }, 500);
}
